...
/------------------------------------------------------------------------------------------------------*/
namespace JLTRY\Component\Jtax\Site\Controller;
...
/**
 * Jtax master site display controller.
 *
 * @since   4.0
 */
class DisplayController extends BaseController
{
    ...
    function display($cachable = false, $urlparams = false)
    {
        ...
        $view          = $this->input->getCmd('view', 'impots');
        ...
        if ($isEdit && !$this->checkEditId($view, $id))
        {
            ...
            if ($refid > 0 && StringHelper::check($ref))
            {
                ...
                $this->setRedirect(Route::_('index.php?option=com_jtax&view=' . (string) $ref . '&layout=edit&id=' . (int) $refid, false));
            }
            elseif (StringHelper::check($ref))
            {
                // redirect to ref
                 $this->setRedirect(Route::_('index.php?option=com_jtax&view=' . (string) $ref, false));
            }
            else
            {
                // normal redirect back to the list default site view
                $this->setRedirect(Route::_('index.php?option=com_jtax&view=impots', false));
            }
    ...
    {
        if (parent::checkEditId("com_jtax.edit.{$context}", $id))
        {
            return true;
        }
        ...
        {
            $this->holdEditId("com_jtax.edit.{$context}", $id);
    ...
    protected function canEditId($view, $id): bool
    {
        ...
        {
            $access = ($this->identity->authorise($allowed['access'], "com_jtax.{$view}." . (int) $id)
                && $this->identity->authorise($allowed['access'], 'com_jtax'));
        }
        ...
        {
            $edit = ($this->identity->authorise($allowed['edit'], "com_jtax.{$view}." . (int) $id)
                && $this->identity->authorise($allowed['edit'], 'com_jtax'));
        }

        // check if this item can be edited by owner
        if ($access && !$edit && isset($allowed['edit.own']))
        {
            $edit = ($this->identity->authorise($allowed['edit.own'], "com_jtax.{$view}." . (int) $id)
                && $this->identity->authorise($allowed['edit.own'], 'com_jtax'));
        }
