...
$edit = "index.php?option=com_jtax&view=years&task=year.edit";
...
<?php foreach ($this->items as $i => $item): ?>
	<?php
		...
		$canDo = JtaxHelper::getActions('year',$item,'years');
	?>
	<tr class="row<?php echo $i % 2; ?>">
		...
		<?php if (!$this->isModal && $canDo->get('core.edit.state')) : ?>
				<?php if ($item->checked_out) : ?>
					...
					<?php else: ?>
						<?php echo Html::_('jgrid.published', $item->published, $i, 'years.', false, 'cb'); ?>
					<?php endif; ?>
				<?php else: ?>
					<?php echo Html::_('jgrid.published', $item->published, $i, 'years.', true, 'cb'); ?>
				<?php endif; ?>
