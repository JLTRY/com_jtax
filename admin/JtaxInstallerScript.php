...
/**
 * Script File of Jtax Component
 *
 * @since  3.6
 */
class Com_JtaxInstallerScript implements InstallerScriptInterface
{
	/**
	 * The CMS Application.
	 *
	...
	public function __construct()
	{
		...
		if (is_file(JPATH_ROOT . '/administrator/components/com_jtax/jtax.php'))
		{
			// remove Joomla 3 files
			$this->deleteFiles = [
				'/administrator/components/com_jtax/jtax.php',
				'/administrator/components/com_jtax/controller.php',
				'/components/com_jtax/jtax.php',
				'/components/com_jtax/controller.php',
				'/components/com_jtax/router.php',
			];
		}

		// check if the Folders exist
		if (is_dir(JPATH_ROOT . '/administrator/components/com_jtax/modules'))
		{
			// remove Joomla 3 folder
			$this->deleteFolders = [
				'/administrator/components/com_jtax/controllers',
				'/administrator/components/com_jtax/helpers',
				'/administrator/components/com_jtax/modules',
				'/administrator/components/com_jtax/tables',
				'/administrator/components/com_jtax/views',
				'/components/com_jtax/controllers',
				'/components/com_jtax/helpers',
				'/components/com_jtax/modules',
				'/components/com_jtax/views',
			];
		}
	}
	...
	public function uninstall(InstallerAdapter $adapter): bool
	{
		...
		// Remove Impot Data
		$this->removeViewData("com_jtax.impot");

		// Remove Year Data
		$this->removeViewData("com_jtax.year");
		...
		<p>Please let me know at <a href="mailto:jltryoen@orange.fr">jltryoen@orange.fr</a>.
		<br />We at JL Tryoen are committed to building extensions that performs proficiently! You can help us, really!
		<br />Send me your thoughts on improvements that is needed, trust me, I will be very grateful!
		<br />Visit us at <a href="http://www.jltryoen.fr" target="_blank">http://www.jltryoen.fr</a> today!</p></div>';
	...
	public function postflight(string $type, InstallerAdapter $adapter): bool
	{
		...
		if ($type === 'install')
		{
			...
			$this->setContentType(
				...
				'',
				// fieldMappings
				'{"common": {"core_content_item_id": "id","core_title": "name","core_state": "published","core_alias": "null","core_created_time": "created","core_modified_time": "modified","core_body": "null","core_hits": "hits","core_publish_up": "null","core_publish_down": "null","core_access": "access","core_params": "params","core_featured": "null","core_metadata": "null","core_language": "null","core_images": "null","core_urls": "null","core_version": "version","core_ordering": "ordering","core_metakey": "null","core_metadesc": "null","core_catid": "null","core_xreference": "null","asset_id": "asset_id"},"special": {"name":"name","taux4":"taux4","taux3":"taux3","taux2":"taux2","taux1":"taux1","tranche4":"tranche4","tranche3":"tranche3","tranche2":"tranche2","tranche1":"tranche1"}}',
				// router
				'',
				// contentHistoryOptions
				'{"formFile": "administrator/components/com_jtax/forms/year.xml","hideFields": ["asset_id","checked_out","checked_out_time"],"ignoreChanges": ["modified_by","modified","checked_out","checked_out_time","version","hits"],"convertToInt": ["published","ordering","version","hits","tranche4","tranche3","tranche2","tranche1"],"displayLookup": [{"sourceColumn": "created_by","targetTable": "#__users","targetColumn": "id","displayColumn": "name"},{"sourceColumn": "access","targetTable": "#__viewlevels","targetColumn": "id","displayColumn": "title"},{"sourceColumn": "modified_by","targetTable": "#__users","targetColumn": "id","displayColumn": "name"}]}'
			);


			// Fix the assets table rules column size.
			$this->setDatabaseAssetsRulesFix(4960, "TEXT");
	...
	protected function setActionLogsExtensions(): void
	{
		...
		$data = new stdClass();
		$data->extension = 'com_jtax';

		// Check if jtax action log extension is already in action logs extensions DB.
		$query = $this->db->getQuery(true);
		$query->select($this->db->quoteName(['id']));
		...
		if ($this->db->getNumRows())
		{
			...
			$this->app->enqueueMessage(
				Text::_('The (com_jtax) is already in the <b>#__action_logs_extensions</b> table.')
			);
		}
		elseif ($this->db->insertObject('#__action_logs_extensions', $data))
		{
			...
			$this->app->enqueueMessage(
				Text::_('The (com_jtax) was successfully added to the <b>#__action_logs_extensions</b> table.')
			);
		}
	}
	...
	protected function setAssetsRules(string $rules): void
	{
		// Condition.
		$conditions = [
			$this->db->quoteName('name') . ' = ' . $this->db->quote('com_jtax')
		];
		...
		if ($done)
		{
			// give a success message
			$this->app->enqueueMessage(
				Text::_('The (com_jtax) rules was successfully added to the <b>#__assets</b> table.')
			);
		}
	}
	...
	protected function setExtensionsParams(string $params): void
	{
		// Condition.
		$conditions = [
			$this->db->quoteName('element') . ' = ' . $this->db->quote('com_jtax')
		];
		...
		if ($done)
		{
			// give a success message
			$this->app->enqueueMessage(
				Text::_('The (com_jtax) params was successfully added to the <b>#__extensions</b> table.')
			);
		}
	}
	 ...
	 *
	 * @param   int     $accessWorseCase   This is the max rules column size com_jtax would needs.
	...
	protected function setDatabaseAssetsRulesFix(int $accessWorseCase, string $dataType): void
	{
		...
		if ($this->db->execute())
		{
			...
			{
				// Fix the assets table rules column size
				$fix = "ALTER TABLE `#__assets` CHANGE `rules` `rules` {$dataType} NOT NULL COMMENT 'JSON encoded access control. Enlarged to {$dataType} by Jtax';";
	...
	{
		// Remove jtax view from the action_log_config table
		$condition = [
			$this->db->quoteName('type_alias') . ' = '. $this->db->quote($context)
		];
		...
		// Execute the query to remove com_jtax.view
		$done = $this->db->execute();
		if ($done)
		{
			// If successfully removed jtax view add queued success message.
			$this->app->enqueueMessage(
	...
	{
		// Remove jtax assets from the assets table
		$condition = [
			$this->db->quoteName('name') . ' LIKE ' . $this->db->quote('com_jtax.%')
		];
		...
		if ($done)
		{
			// If successfully removed jtax add queued success message.
			$this->app->enqueueMessage(
				Text::_('All related (com_jtax) items was removed from the <b>#__assets</b> table.')
			);
		}
	}
	...
	{
		// Remove jtax from the action_logs_extensions table
		$extension = [
			$this->db->quoteName('extension') . ' = ' . $this->db->quote('com_jtax')
		];
		...
		// Execute the query to remove jtax
		$done = $this->db->execute();
		if ($done)
		{
			// If successfully remove jtax add queued success message.
			$this->app->enqueueMessage(
				Text::_('The (com_jtax) extension was removed from the <b>#__action_logs_extensions</b> table.')
			);
		}
	}
	...
	protected function classExists(string $className): bool
	{
		...
		$autoloaders = [
			__DIR__ . '/JtaxInstallerPowerloader.php',
			JPATH_ADMINISTRATOR . '/components/com_jtax/src/Helper/PowerloaderHelper.php'
		];
