...
class ImpotModel extends AdminModel
{
	...
	protected array $styles = [
		...
		'administrator/components/com_jtax/assets/css/impot.css'
 	];
	...
	public function getForm($data = [], $loadData = true, $options = ['control' => 'jform'])
	{
		...
		// Get the form.
		$form = $this->loadForm('com_jtax.impot', 'impot', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_jtax.impot.' . (int) $id))
...
/***[JCBGUI.admin_view.php_getform.288.$$$$]***/
...
$language->load('joomla', JPATH_ADMINISTRATOR);/***[/JCBGUI$$$$]***/
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('core.edit.state', 'com_jtax.impot.' . (int) $recordId);
	...
	protected function allowEdit($data = [], $key = 'id')
	{
		...
		return Factory::getApplication()->getIdentity()->authorise('core.edit', 'com_jtax.impot.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo = JtaxHelper::getActions('impot');
		$this->batchSet = true;
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.create') && !$this->canDo->get('impot.batch'))
		{
			return false;
		}
		...
		while (!empty($pks))
		{
			...
			// Only for strings
			if (UtilitiesStringHelper::check($this->table->year) && !is_numeric($this->table->year))
			{
				$this->table->year = $this->generateUnique('year',$this->table->year);
			}
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (!$this->canDo->get('core.edit') && !$this->canDo->get('impot.batch'))
		{
