 ...
 * @since  1.6
 */
class YearsModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_jtax/assets/css/years.css'
 	];
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__jtax_year', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = Factory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__jtax_year");
	...
	protected function checkInNow(): bool
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__jtax_year'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__jtax_year'))->set($fields)->where($conditions); 
