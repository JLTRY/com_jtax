...
/------------------------------------------------------------------------------------------------------*/
namespace JLTRY\Component\Jtax\Administrator\Table;
 ...
 * @since  1.6
 */
class ImpotTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
    ...
    function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
    {
        ...
        parent::__construct('#__jtax_impot', 'id', $db, $dispatcher);
    }
    ...
    /**
     * Overload the store method for the Impot table.
     *
    ...
    public function store($updateNulls = true)
    {
        ...
        {
            // New impot. A impot created and created_by field can be set by the user,
        ...
        if (isset($this->alias))
        {
            ...
            {
                $this->setError(Text::_('COM_JTAX_IMPOT_ERROR_UNIQUE_ALIAS'));

                if ($table->published === -2)
                {
                    $this->setError(Text::_('COM_JTAX_IMPOT_ERROR_UNIQUE_ALIAS_TRASHED'));
                }
    ...
    public function check()
    {
        ...
        if (!$this->getRules())
        {
            $rules = $this->getDefaultAssetValues('com_jtax.impot.'.$this->id);
            $this->setRules($rules);
        }
