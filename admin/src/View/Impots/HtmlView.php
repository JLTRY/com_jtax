...
/------------------------------------------------------------------------------------------------------*/
namespace JCB\Component\Jtax\Administrator\View\Impots;
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canBatch = ($this->canDo->get('impot.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_JTAX_IMPOTS'), 'arrow-down-3');

		if ($this->canCreate)
		{
			ToolbarHelper::addNew('impot.add');
		}
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('impot.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('impots.publish');
				ToolbarHelper::unpublishList('impots.unpublish');
				ToolbarHelper::archiveList('impots.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('impots.checkin');
				}
			}

			if ($this->state->get('filter.published') == -2 && ($this->canState && $this->canDelete))
			{
				ToolbarHelper::deleteList('', 'impots.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('impots.trash');
			}
		}
	...
	protected function _prepareDocument(): void
	{
		...
		$this->getDocument()->setTitle(Text::_('COM_JTAX_IMPOTS'));
		// add styles
