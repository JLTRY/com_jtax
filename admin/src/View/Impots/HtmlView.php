...
/------------------------------------------------------------------------------------------------------*/
namespace JLTRY\Component\Jtax\Administrator\View\Impots;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar(): void
    {
        ...
        if (!$this->isEmptyState)
        {
            ...
            if ($this->canEdit)
            {
                $childBar->edit('impot.edit')->listCheck(true);
            }

            if ($this->canState)
            {
                $childBar->publish('impots.publish')->listCheck(true);
                $childBar->unpublish('impots.unpublish')->listCheck(true);
                $childBar->archive('impots.archive')->listCheck(true);

                if ($this->canDo->get('core.admin'))
                {
                    $childBar->checkin('impots.checkin')->listCheck(true);
                }

                if ($this->state->get('filter.published') == -2 && $this->canDelete)
                {
                    $toolbar->delete('impots.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
                        ->message('JGLOBAL_CONFIRM_DELETE')
                        ->listCheck(true);
                }
                elseif ($this->canDelete)
                {
                    $childBar->trash('impots.trash')->listCheck(true);
                }
            }
        }
